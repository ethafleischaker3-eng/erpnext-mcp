...
| 版本 | v1.0 |
| 状态 | 已冻结 |
| 创建人 | Claude（B03 起草上下文） |
| Owner | gjg |
| Implementer | Claude（B03 独立实施上下文）；须与 Acceptor 为不同的执行实例并验证隔离 |
| Implementation Reviewer | gjg；不得读取后续对应隐藏验收材料 |
| Acceptance Reviewer | 不适用；B03 不接触正式验收材料（不生成/读取验收题、隐藏数据或终态断言） |
| Acceptor | gjg；独立于 Implementer |
| 创建日期 | 2026-09-15 |
| 冻结日期 | 2026-09-15 |
| 完成日期 | 不适用 |
| 上游任务包 | A01（已封存，2026-09-14，启动门槛）；B00（已封存，提供环境/身份/权限/币种价格表事实）；B01（已封存，提供主数据接口事实与引用删除说明）；B02（已封存，提供销售链路接口事实、乐观版本断言与原子性结论） |
| 下游任务包 | D01（公共契约与错误模型，其冻结依赖 B01～B05 接口事实）；经 D01→D02 传递至 F03（采购业务闭环） |

角色隔离说明：Implementer 为独立编码上下文，Acceptor 为 gjg，二者为不同的执行实例；B03 实施采购链路接口摸底，不实施任何正式业务 tool、不读取正式验收题。Implementation Reviewer gjg 兼任 Owner，但不替代独立验收。接口事实记录的「接入方确认有效」由 Owner/决策人 gjg 在独立验收环节签署，与 Implementer 自证相分离。

## 2. 目标

在验收环境 `erpnext.local` 上，用 `B03-PROBE-` 前缀合成对象实测 ERPNext 采购链路单据（Purchase Order、Purchase Receipt）的接口行为，产出可复核的接口事实记录并冻结结论，作为 D01/D02 业务 tool 契约（采购订单、采购收货单 tool）的权威输入。验证记录必须逐项覆盖：

1. **对象与接口基础可用性**：Purchase Order / Purchase Receipt 的 create/confirm/cancel 端点齐备且可达；Purchase Receipt 按来源采购订单生成草稿的方式；草稿删除作为回滚路径确认（PRD §5.4）；
2. **命名与编号**：`naming_series` 自动编号行为、`name` 生成形式（推测：采购订单 `PUR-ORD-.YYYY.-`、采购收货单 `MAT-PRE-.YYYY.-`，为 series 连续编号、无「同名唯一性」，待实测确认）；
3. **引用字段有效性**：supplier / item / warehouse / company / currency / price list（buying）等 Link 字段无效时的校验行为；
4. **单据状态机与 docstatus**：create→草稿（docstatus=0）、confirm→已生效（1）、cancel→已取消（2）的状态漂移及 `modified` 变化语义；
5. **乐观版本断言**：confirm / cancel 时传入陈旧 `modified` 是否被后端拒绝、拒绝形式（PRD §5.2 #17/#18/#20）；
6. **来源单据与数量/库存校验**：Purchase Receipt 来源订单必须已生效、来源行剩余可收数量、confirm 收货入库的库存增加与仓库有效校验行为（PRD §5.2 #20）；
7. **原子性**：create / confirm / cancel 是否由后端单次原子完成，confirm 状态校验与提交是否同一事务（PRD §5.5）；
8. **回滚/取消条件**：cancel 的合法前态、下游单据约束、已生效单据取消的冲回行为（Purchase Receipt cancel 冲回库存）；草稿删除路径（PRD §5.4）；
...
**前置数据与库存边界**：本包允许创建 `B03-PROBE-` 前缀 Supplier / Supplier Group / Item 作为采购单据的引用前置数据（复用 B01 已冻结主数据接口事实，不重复摸底）。Purchase Receipt confirm 的「收货入库」会产生**真实库存增加**（Stock Ledger Entry 正向 + Bin 余量上升），这正是 B02 留待的「充足库存正向发货」的前置来源；本包实测「收货入库 → 库存增加」正向行为与「库存冲回 → 零残留」清理路径，不测销售出库（F02 范围），也**不测采购退货单（Purchase Return）**（本 PRD 不纳入——tool 清单无采购退货 tool；本包仅测正向收货与 cancel 冲回）。**注：Purchase Receipt cancel 仅作库存冲回与零残留清理摸底，不构成 D02 新增 `purchase_receipt_cancel` tool 的依据（PRD §3.3 与已决议 #8：业务取消仅 `sales_order_cancel`/`purchase_order_cancel`）。**

## 3. 非目标

- 不开发 MCP server 或任何正式业务 tool（F03 职责）；
- 不冻结 tool 契约、不定义 tool 语义/输入输出 schema/annotation/错误结构（D01/D02 职责）；
- 不形成最终业务权限、正式角色矩阵或两张允许清单（E01 职责）；
- 不开展 B01 主数据 / B02 销售链路 / B04 库存与回滚链路 / B05 幂等可实现性调研；
- 不实现幂等机制、批次台账、前置断言、事后校验或回滚逻辑（E02–E04 职责）；
- 不生成、读取或运行正式验收题、隐藏数据、终态断言或评分脚本；
- 不修改 ERPNext 源码、schema、`frappe_docker` 或站点配置；
- 不测采购退货单（Purchase Return，本 PRD 不纳入——tool 清单无采购退货 tool）、不测销售出库、不测库存调拨/盘点；
- 不以 B03 通过代替后续契约冻结、业务 tool 实现或项目验收。
...
| B02 v1.0 已封存 | 检查 B02 登记表行、Freeze Manifest 及 `governance-b02-v1.0-accepted` 标签 | 已验证 |
...
| 本地 ERPNext 实例可访问且凭据可用 | 复核 `http://localhost:8080` 可达、Token 与登录凭据有效（B03 实施时复核） | 未验证 |
| 站点 `erpnext.local` 为独占、非生产、可销毁验收环境，快照可重置 | 复用 B00/B01/B02 已冻结环境事实，实施时复核站点独占与快照重置可用 | 未验证 |
| 供应商主数据就位（Supplier Group、默认 buying Price List） | 复核 `Standard Buying` 价格表与 Supplier Group 是否可用（B02 W01 已确认存在 2 条 Price List：Standard Selling + Standard Buying；Supplier Group 待实测确认） | 未验证 |
...
| 13 | 治理修订评审记录 | `docs/governance/changes/变更记录-2026-09-12-任务包治理修订.md` | CHG-20260912-001（2026-09-12） |

## 6. 授权范围与所有权

### 6.1 可读取范围

- `docs/` 中的规范、PRD、治理文件、P00/B00/A01/B01/B02 材料及公开记录；
- `erp/README.md` 及 `frappe_docker/` 下的部署配置（`compose.yaml`、`overrides/`、`example.env` 等）；
- `frappe_docker/` 内的 ERPNext/Frappe 后端源码与数据模型（用于核对 Purchase Order / Purchase Receipt 字段、状态机、校验逻辑与版本断言行为）；
- ERPNext 运行时：只读 bench 命令、采购链路对象范围的 REST API 调用、只读数据库查询；
...
|---|---|---|---|---|
| `docs/task-packages/B03/` | 新增和维护 B03 任务包、公开证据、接口事实记录与 Evidence Manifest | gjg | 否 | 状态变化必须追加记录，不覆盖历史 |
| `docs/governance/任务包登记表.md` 的 B03 行 | 更新 B03 版本、状态、角色、路径与证据位置 | gjg | 是 | 仅更新 B03 行；其他任务行实质变化另走相应任务或变更流程 |
| `docs/task-records/changes/`、`returns/` | 保存 B03 变更或退回记录（如发生） | gjg | 是 | 使用稳定编号和独立文件 |
| `docs/task-records/freeze-manifests/B03-v1.0.md` | 冻结时登记 B03 冻结清单与哈希 | gjg | 否 | 哈希针对冻结文件计算，不写回被哈希文件 |
| 验收区 `D:\second-acceptance\evidence\` | 保存 B03 敏感原始输出（数据库查询结果、含凭据痕迹的 REST 响应） | gjg | 是 | 脱敏；仅本地验收环境；保留至项目交付 |
| 后端合成采购单据 | 创建/confirm/cancel/删除 `B03-PROBE-` 标识的 Purchase Order、Purchase Receipt（仅验收环境） | gjg | 否 | 单据 name 为 series 自动编号，合成标识经供应商/物料承载；每次测试后清理，纳入 Evidence Manifest |
| 后端合成前置主数据 | 创建/删除 `B03-PROBE-` 前缀 Supplier、Supplier Group、Item（作为采购单据引用前置；复用 B01 主数据接口事实，不重复摸底） | gjg | 否 | 唯一前缀避免冲突；每次测试后清理，纳入 Evidence Manifest |

### 6.3 系统、接口、数据与环境权限

| 权限类别 | 允许范围/对象 | 允许动作 | 明确禁止 | 是否可改变状态 | 不适用理由/审批与证据 |
|---|---|---|---|---|---|
| 系统、接口与命令 | 本地 Docker Compose 生命周期；`docker exec` 只读 bench；`curl` 后端 REST API（限 Purchase Order/Purchase Receipt 读写 + 引用对象只读 + `B03-PROBE-` Supplier/Item 前置数据创建/删除）；`mariadb` 只读查询与快照 dump/restore | 启动/停止容器、执行只读 bench、调用采购链路 REST API、dump/restore 站点数据库 | 禁止调用清单外对象写接口；禁止修改 `frappe_docker` 源码或提交历史；禁止 `git push`、历史改写 | 是，仅验收环境后端状态 | 由本任务交付物与证据证明 |
...
- 不得扩大对象、tool 或权限范围（仅 Purchase Order/Purchase Receipt 读写 + 引用对象只读 + `B03-PROBE-` 前置 Supplier/Item 创建/删除）；
- 不得重新验证 B01 已冻结的主数据结论（同名/版本/区间），`B03-PROBE-` Supplier/Item 仅作引用前置；
- 不得重新验证 B02 已冻结的销售链路结论，仅复用其「乐观版本断言 / 原子性 / 状态机」的实测方法与前置事实；
...
- 不得让 `B03-PROBE-` 合成对象在测试后残留（每次测试后删除或快照恢复；Purchase Receipt confirm 产生的库存须经 cancel 冲回或快照恢复归零）；
- 不得测采购退货单（本 PRD 不纳入——tool 清单无采购退货 tool）、销售出库（F02 范围）、库存调拨/盘点（B04/F04 范围）；
- 不得将 B03 状态误作契约冻结、业务 tool 实现或项目完成状态。

## 8. 工作项

| 编号 | 工作项 | 交付物/完成断言 |
|---|---|---|
| W01 | 复核验收环境与凭据 | 复核 `erpnext.local` 独占非生产可销毁、版本符合冻结目标 `15.121.2`/`15.120.1`、快照重置可用、凭据有效（脱敏）；复用 B00/B01/B02 已冻结事实并记录复核结论 |
| W02 | Purchase Order create 摸底 | 实测 Purchase Order create（naming_series 自动编号、supplier/item/price list（buying）引用校验、草稿 docstatus=0、子表行项目嵌套读写）；记录接口路径、行为与报错形式 |
| W03 | Purchase Order confirm 摸底 | 实测 confirm（状态漂移 0→1、modified 变化、陈旧 modified 版本断言、供应商/物料/日期/价格关键状态校验）；记录结论 |
| W04 | Purchase Order cancel 摸底 | 实测 cancel（合法前态、下游单据约束、已生效取消的冲回行为、陈旧 modified 版本断言）；记录结论 |
| W05 | Purchase Receipt create 摸底 | 实测按来源采购订单生成收货草稿（来源订单须已生效、行数量不超过未完成量、子表行项目、草稿状态）；记录结论 |
| W06 | Purchase Receipt confirm 摸底 | 实测 confirm（状态漂移、陈旧 modified 版本断言、来源行剩余可收数量校验、收货入库 → 库存增加（Stock Ledger Entry + Bin））；记录结论 |
| W07 | 原子性/失败不静默与清理确认 | 实测 create/confirm/cancel 是否单次原子完成、confirm 状态校验与提交是否同一事务（PRD §5.5）；写失败是否被正确报告（非静默成功）；确认 `B03-PROBE-` 对象清理路径与零残留（含收货入库产生的库存冲回归零） |
| W08 | 冻结接口事实记录 | 汇总 W01–W07 形成接口事实记录（覆盖第 9 节全部 9 项），作为 D01/D02 权威输入 |

## 9. 输入输出契约

B03 不提供正式业务 tool，不定义 tool 语义或 schema。本节规定接口事实记录的冻结输出结构（D01/D02 权威输入）。

接口事实记录（交付物 D04）必须逐对象（Purchase Order / Purchase Receipt）逐项包含并可复核：

1. 接口路径与可用性：create/confirm/cancel 的 REST 端点或等效调用方式、实测可达性；Purchase Receipt 按来源采购订单生成草稿的方式；草稿删除作为回滚路径确认（PRD §5.4）；
2. 命名与编号：`naming_series` 自动编号行为、`name` 生成形式（推测：`PUR-ORD-.YYYY.-` / `MAT-PRE-.YYYY.-`，为 series 连续编号、无「同名唯一性」，待实测确认）；
3. 引用字段有效性：supplier / item / warehouse / company / currency / price list（buying）等 Link 字段无效时的校验行为；
4. 单据状态机与 docstatus：create→草稿（docstatus=0）、confirm→已生效（1）、cancel→已取消（2）的状态漂移及 `modified` 变化语义；
5. 乐观版本断言：confirm / cancel 时传入陈旧 `modified` 是否被后端拒绝、拒绝形式（PRD §5.2 #17/#18/#20）；
6. 来源单据与数量/库存校验：Purchase Receipt 来源订单须已生效、来源行剩余可收数量、confirm 收货入库的库存增加与仓库有效校验行为（PRD §5.2 #20）；
7. 原子性结论：create/confirm/cancel 是否由后端单次原子完成，confirm 状态校验与提交是否同一事务（PRD §5.5）；
8. 回滚/取消条件：cancel 的合法前态、下游单据约束、已生效取消的冲回行为（Purchase Receipt cancel 冲回库存——仅作清理摸底，不构成 D02 新增 `purchase_receipt_cancel` tool 依据，PRD §3.3 / 已决议 #8）；草稿删除路径（PRD §5.4）；
9. 失败不静默：写失败是否被正确报告（非静默成功）。

记录一经冻结，写入 PRD 或成为 D01/D02 权威输入时均须走变更控制；B03 本身不改变 PRD 冻结语义。

## 10. 不变量

- 冻结 PRD、总则、模板、登记表结构与边界规则内容不得被 B03 修改；
- 不修改 ERPNext 源码、schema、`frappe_docker` 或站点配置；
- 仅操作 `B03-PROBE-` 标识合成采购单据与前置主数据，不写清单外对象或真实业务数据；
- 每次测试后合成对象不残留（删除或快照恢复），含 Purchase Receipt confirm 产生的库存（经 cancel 冲回或快照恢复归零）；
- 写失败不得以成功响应伪装；
...
| 编号 | 交付物 | 存放位置 | 验收方式 |
|---|---|---|---|
| D01 | B03 任务包 | `docs/task-packages/B03/task.md` | 按冻结模板检查必备结构 |
| D02 | B03 Evidence Manifest | `docs/task-packages/B03/evidence-manifest.md` | 逐项追溯工作项与完成定义 |
| D03 | 更新后的任务包登记表 | `docs/governance/任务包登记表.md` | B03 行与本文件一致，表格结构未改变 |
| D04 | 采购链路接口事实记录 | `docs/task-packages/B03/interface-facts.md` | 覆盖第 9 节全部 9 项，逐对象可复核 |
| D05 | 合成数据与清理记录 | `docs/task-packages/B03/evidence/`（脱敏）+ `D:\second-acceptance\evidence\`（敏感） | 合成对象唯一前缀、清理终态可复核 |
| D06 | 各场景原始证据（REST 响应、数据库查询、报错，脱敏） | `docs/task-packages/B03/evidence/` | 逐场景可复核 |
...
| 编号 | 检查方法 | 预期结果 |
|---|---|---|
| S01 | 复核站点独占/非生产/版本/快照重置/凭据 | 环境事实与 B00/B01/B02 冻结一致；快照可重置；凭据有效 |
| S02 | Purchase Order create 合成对象实测 | naming_series 编号、引用校验、草稿状态、子表行项目均有实测记录 |
| S03 | Purchase Order confirm 合成对象实测 | 状态漂移、版本断言、关键业务状态校验均有实测记录 |
| S04 | Purchase Order cancel 合成对象实测 | 合法前态、下游约束、冲回行为、版本断言均有实测记录 |
| S05 | Purchase Receipt create 合成对象实测 | 来源订单已生效校验、未完成量校验、子表行项目均有实测记录 |
| S06 | Purchase Receipt confirm 合成对象实测 | 状态漂移、版本断言、剩余可收数量、收货入库库存增加均有实测记录 |
| S07 | 原子性/失败不静默/清理测试 | 单次原子完成；失败不静默成功；`B03-PROBE-` 对象与收货入库库存测试后零残留 |
| S08 | 汇总接口事实记录 | 覆盖第 9 节全部 9 项，逐对象逐项可复核 |

## 13. Evidence Manifest

证据统一维护在 `docs/task-packages/B03/evidence-manifest.md`，本文件不重复记录明细。
...
继承《MCP 改造任务包总则》第 6.15 节。另有以下 B03 条件时，停止受影响工作并转为「待澄清」：

- 本地 ERPNext 实例不可达且无法修复；
- 站点 `erpnext.local` 无法确认为独占、非生产、可销毁，或快照无法重置；
- 采购链路对象接口无法在授权范围内实测（如端点缺失、权限不足）；
- 需要新增对象、tool、权限或外部依赖；
- 需要修改冻结 PRD、总则、模板、登记表结构或边界规则。

**注意**：「乐观版本断言 / confirm 状态漂移 / 原子性不被后端原生保证」不是停止条件，而是可判定的有效结论——此时结论为「须由 MCP server 层实现」，仍作为 D01/D02 权威输入（PRD §5.2/§5.5）。只有当无法取得可复核的确定性结论时，才触发停止与升级。

报告必须包含阻断事实、影响范围、已完成的安全检查以及需要谁作出什么决定。

## 16. 风险、假设与待决事项

- 合成对象统一使用 `B03-PROBE-` 标识；单据 `name` 为 naming_series 自动编号，合成标识经供应商/物料承载，每次测试后以 create 返回的 `name` 列表（PUR-ORD-XXXXX/MAT-PRE-XXXXX）作清理主键删除或快照恢复；清理注意顺序——已确认的 PO 存在下游 PR 时先 cancel→delete PR、再 cancel→delete PO；Purchase Receipt confirm 产生的库存须先 cancel 冲回（或快照恢复）再删单据；
...
- Supplier / Supplier Group 主数据就位情况待实测（`Standard Buying` 价格表已由 B02 W01 确认；Supplier Group 是否存在、是否需本包创建待复核）；
- Purchase Receipt confirm 收货入库会产生真实库存增加（Stock Ledger Entry + Bin），清理须确保库存冲回归零；「采购退货」本 PRD 不纳入（tool 清单无采购退货 tool）；
- cancel 的下游单据约束行为待实测（已生效采购订单存在下游收货单时是否拒绝取消、拒绝形式）；
- 前置 Supplier/Item 合成数据复用 B01 已冻结主数据接口事实创建，不重复摸底、不得重新验证 B01 已冻结的主数据结论（同名/版本/区间）；
- Implementer 为独立编码上下文，Acceptor=gjg、Implementation Reviewer=gjg；实施会话与 Acceptor 的隔离沿用 B00 已建立的隔离事实（受限账户 + NTFS ACL），B03 不重复建设；
