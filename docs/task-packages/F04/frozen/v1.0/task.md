...
> 文档版本：v1.0；状态：已冻结；Reviewer：gjg；决策人：gjg；冻结日期：2026-09-17。
...
> 本节在档位 3 独立验收通过后回填（档位 3 强制：冻结 → 实施 → 独立验收 → 封存 + 下游影响记录 + Freeze Manifest，总则 §7.1）。预填下游影响口径：F04 封存后为 F02（直接下游）提供 10 个写/只出 plan tool（#6–#12 主数据 + #23–#25 库存）与复用骨架；F02 须复用 F01/F04 建立的 `server/` 骨架（入口/注册表只增不改、串行追加 #13–#22），并以 mcp-service 为后端调用方、挂接 E01 确认/E02 机制（总则 §16.3 写操作门槛）；G01 完整集成验收以 E/F 全部完成（含 F04）为前置。
