...
| 本地 ERPNext 实例可访问且凭据可用 | 复核 `http://localhost:8080` 可达、Token 与登录凭据有效（B02 实施时复核） | 未验证 |
| 站点 `erpnext.local` 为独占、非生产、可销毁验收环境，快照可重置 | 复用 B00/B01 已冻结环境事实，实施时复核站点独占与快照重置可用 | 未验证 |
