...
> 文档版本：v1.0；状态：已通过；Reviewer：gjg；决策人：gjg；冻结日期：2026-09-16。
...
| 实施区/验收区实际路径已填写、隔离已生效（Implementer 会话对 `task-sets/`、`assertions/` 只读拒绝） | 负向读取自证 + ACL 核查 | 已验证（2026-09-16 实施前自证：verify-impl.ps1 返回 `ISOLATED: all checks passed`，S01 四目录 ACCESS_DENIED） |
| 本地验收环境可读（ERPNext 15.121.2 / Frappe 15.120.1；mcp-service 可读目标对象，只读不写） | 检查 B00/E01 结论与快照重置能力 | 已验证（2026-09-16：后端 localhost:8080 可达、空基线；只读管线冒烟通过，全 GET、未改数据库状态；mcp-service 账号/角色已由 E01 落地） |
...
| 2026-09-16 | 已冻结 | 实施中 | Claude（F01 实施上下文） | 开工前五步完成：依序核对权威输入（D02/D01/E01/E02/C01a/B01–B04/PRD/规范/总则/边界）、核对 §4 前置、检查工作区无未授权变更、声明任务边界（只做 F01，不实现写 tool、不实现 #25、不读 C01b）、隔离验证 `verify-impl.ps1` 返回 `ISOLATED: all checks passed` |
| 2026-09-16 | 实施中 | 待验收 | Claude（F01 实施上下文） | W02–W09 完成：W02 隔离自证（task-sets/assertions/runs/snapshots 负向读取 ACCESS_DENIED）；W03 实现 #1/#2/#3/#4/#5 契约层；W04 实现 #26 挂接 E02 queryBatchStatus；W05 确认 #1 filters 白名单（交付物 D03）；W06 挂接 E01 白名单层；W07 挂接 E02 错误转译与「不适用」标注；W08 D02/D01/E01 对齐核对（D04）；W09 自检 S01–S10 75/75 通过 + Evidence Manifest 回填（D05/D06）。未触发 §15 停止条件。提交待验收（档位 2 独立验收归 Acceptor gjg，Implementer 不自验、不标「已通过」） |
| 2026-09-16 | 待验收 | 已通过 | gjg（Acceptor） | 档位 2 独立验收通过（终态「已通过」，封存不强制单独进行，下游影响记录并入 P00 或 F04）。逐 tool 核对 6 读 tool 与 D02 §2.1–§2.6、E01 白名单层、E02 机制挂接、只读口径一致；frozen 快照完整（仅追加状态记录）；独立重跑自检 S02–S10 71/71 通过（S01 隔离项在 Acceptor 账户下按预期不成立，实施侧 b00-impl 证据 75/75）；无清单外对象、无越界读写、无夹带写入；未读 C01b 冻结任务集正文/断言。验收记录 D08 见 acceptance-record.md |
